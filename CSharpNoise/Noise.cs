    ...
    public static class Noise
    {
...
#if CORECLR
        public static unsafe void QuadraticNoise2D(Span<float> xCoords, Span<float> yCoords, Span<float> output, float xFreq, float yFreq, float amplitude, int seed)
        {
            ...
            for (int i = 0; i < length - Float.Count; i += Float.Count)
            {
                ...
                Float result = QuadraticNoise2DVector(xVec, yVec, seedVec) * amplitude;
            ...
            Float resultEnd = QuadraticNoise2DVector(xEnd, yEnd, seedVec) * amplitude;
        ...
        public static unsafe void QuadraticNoise3D(Span<float> xCoords, Span<float> yCoords, Span<float> zCoords, float xFreq, float yFreq, float zFreq, float amplitude, int seed, Span<float> output)
        {
            ...
            for (int i = 0; i < length - Float.Count; i += Float.Count)
            {
                ...
                Float result = QuadraticNoise3DVector(xVec, yVec, zVec, seedVec) * amplitude;
                result.StoreUnsafe(ref output[i]);
            }
            int endIndex = length - Float.Count;
            Float xEnd = Util.LoadUnsafe(ref xCoords[endIndex]) * xfVec;
            ...
            Float resultEnd = QuadraticNoise3DVector(xEnd, yEnd, zEnd, seedVec) * amplitude;
            resultEnd.StoreUnsafe(ref output[endIndex]);
            }
#else
        /// <summary>
...
#if QUADRATIC
            llbGrad = Util.MultiplyAddEstimate(llbGrad, llbGrad * Util.AsVectorSingle(Util.AsVectorInt32(negOne) ^ (llbHash & (1 << 31))), llbGrad);
            lrbGrad = Util.MultiplyAddEstimate(lrbGrad, lrbGrad * Util.AsVectorSingle(Util.AsVectorInt32(negOne) ^ (lrbHash & (1 << 31))), lrbGrad);
#endif
...
#if QUADRATIC
            ulbGrad = Util.MultiplyAddEstimate(ulbGrad, ulbGrad * Util.AsVectorSingle(Util.AsVectorInt32(negOne) ^ (ulbHash & (1 << 31))), ulbGrad);
            urbGrad = Util.MultiplyAddEstimate(urbGrad, urbGrad * Util.AsVectorSingle(Util.AsVectorInt32(negOne) ^ (urbHash & (1 << 31))), urbGrad);
#endif
...
#if QUADRATIC
            llfGrad = Util.MultiplyAddEstimate(llfGrad, llfGrad * Util.AsVectorSingle(Util.AsVectorInt32(negOne) ^ (llfHash & (1 << 31))), llfGrad);
            lrfGrad = Util.MultiplyAddEstimate(lrfGrad, lrfGrad * Util.AsVectorSingle(Util.AsVectorInt32(negOne) ^ (lrfHash & (1 << 31))), lrfGrad);
#endif
...
#if QUADRATIC
            ulfGrad = Util.MultiplyAddEstimate(ulfGrad, ulfGrad * Util.AsVectorSingle(Util.AsVectorInt32(negOne) ^ (ulfHash & (1 << 31))), ulfGrad);
            urfGrad = Util.MultiplyAddEstimate(urfGrad, urfGrad * Util.AsVectorSingle(Util.AsVectorInt32(negOne) ^ (urfHash & (1 << 31))), urfGrad);
#endif
